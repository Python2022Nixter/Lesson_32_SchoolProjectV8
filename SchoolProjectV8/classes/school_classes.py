...
class School:
    ...
    def __init__(self) -> None:
        ...
        self.last_message = ""
        self.PATH_TO_FILE_DATABASE = pathlib.Path(__file__).parent.parent.joinpath(
            c.FOLDER_DATABASE).joinpath(c.FILE_DATABASE)
        self.PATH_TO_CSV_PERSONS = pathlib.Path(__file__).parent.parent.joinpath(
            c.FOLDER_CSV).joinpath(c.FILE_CSV_PERSONS)
        self.PATH_TO_CSV_COURSES = pathlib.Path(__file__).parent.parent.joinpath(
            c.FOLDER_CSV).joinpath(c.FILE_CSV_COURSES)
        self.PATH_TO_CSV_POSITIONS = pathlib.Path(__file__).parent.parent.joinpath(
            c.FOLDER_CSV).joinpath(c.FILE_CSV_POSITIONS)
        pass
    ...
    def __login(self):
        ...
        try:
            ...
            with sqlite3.connect(self.PATH_TO_FILE_DATABASE) as conn:
                ...
                res = cursor.execute(c.SQL_CHECK_TABLE_EXISTS, [
                                     c.TABLE_NAME_PERSONS])
                ...
                if table_found:
                    ...
                    if admins_found:
                        while True:  # BEGIN login loop
                            ...
                            print(len(res) != 0)
                            if len(res) != 0:
                                # call class constructor: pass tuple with single user data to class constructor
                                self.__registered_user = Person(res[0])
                                print(111)
                                break
                ...
                while True:
                    ...
                    match input(menu_string):
                        ...
                        case "3":
                            ...
                            self.__create_table(c.SQL_CREATE_TABLE_PERSONS)
                            self.__import_from_csv(
                                path_to_csv=self.PATH_TO_CSV_PERSONS, sql=c.SQL_ADD_RECORD_PERSONS)
                            pass
                        case "4":
                            # Drop table: PERSONS
                            self.__drop_table(c.SQL_DROP_TABLE_PERSONS)
                        ...
                        case "8":
                            ...
                            self.__create_table(c.SQL_CREATE_TABLE_POSITIONS)
                            self.__import_from_csv(
                                self.PATH_TO_CSV_POSITIONS, c.SQL_ADD_RECORD_POSITIONS)
                            pass
                        case "9":
                        ...
                        case "10":
                            ...
                            self.__update_table(c.SQL_UPDATE_TABLE_POSITIONS)
                        ...
                        case "13":
                            ...
                            self.__import_from_csv(
                                self.PATH_TO_CSV_COURSES, c.SQL_ADD_RECORD_COURSES)
                            pass
                        case "14":
                            # Drop table: COURSES
                            self.__drop_table(c.SQL_DROP_TABLE_COURSES)
                        ...
                        case "15":
                            ...
                            self.__update_table(c.SQL_UPDATE_TABLE_COURSES)
    ...
    def __import_from_csv(self, path_to_csv: str, sql: str) -> None:
        ...
        with open(path_to_csv) as f:
            rows_from_file = list(
                filter(lambda x: x[0].isdigit(), f.readlines()))

        values_list = []
        for n_row in rows_from_file:
            values_list.append(n_row.strip().split(",")[1:])
        # populate DB table
        ...
        for i in range(len(table_columns)):
            if i == len(table_columns) - 2:
                ...
                while True:
                    ...
                    if entered_position_id in id_list:
                        break
                    print(
                        F"{c.SET_ERROR_MESSAGE_COLOR}Incorrect position id{c.RESET_COLOR}\n")
                    pass
                ...
                while True:
                    ...
                    if entered_course_id in id_list:
                        break
                    print(
                        F"{c.SET_ERROR_MESSAGE_COLOR}Incorrect course id{c.RESET_COLOR}\n")
                    pass
                ...
                while True:
                    ...
                    if entered_login not in login_list:
                        break
                    print(
                        F"{c.SET_ERROR_MESSAGE_COLOR}Selected login name in use already{c.RESET_COLOR}\n")
                    pass
    ...
    # TO DO -> Remove method
    def __create_table(self, sql: str) -> None: self.__execute_sql_query(sql)

    # TO DO -> Remove method
    def __drop_table(self, sql: str) -> None: self.__execute_sql_query(sql)
    ...
    def __update_table(self, sql: str) -> None:
        ...
        # menu - input field name to update
        table_name = sql.split()[1].replace("\"", "")  # get tab

        change_menu = "Select field name to change in: \n"
        filter_menu = "Select filter field name: \n"
        match table_name:
        ...
        for ind, next_item in enumerate(fields_list[1:]):
            # F"{c.SET_HIGHLIGHT_TEXT_COLOR}{ind:>2d}{c.RESET_COLOR} - {next_item}"
            change_menu += F"{c.SET_HIGHLIGHT_TEXT_COLOR}{ind+1:3d}{c.RESET_COLOR} - {next_item}\n"
            if ind == 0: continue
        ...
        field_name_to_change = input(change_menu)
        field_name_to_filter = input(filter_menu)

        params = []
        sql.replace("__FIELD_NAME_TO_CHANGE__", field_name_to_change).replace(
            "__FIELD_NAME_TO_FILTER__", field_name_to_filter)
        # UPDATE "{TABLE_NAME_PERSONS}" SET "last_name=? WHERE "filter_field_name" = ?;
    ...
    def __execute_sql_query(self, query_string: str, query_parameters: list | None = None) -> str | None:
        ...
        try:
            ...
            with sqlite3.connect(self.PATH_TO_FILE_DATABASE) as conn:
                ...
                match (query_string.split()[0], query_parameters):
                    case ("SELECT", None):
